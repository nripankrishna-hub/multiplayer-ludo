  ...
  moveToken(color, tokenId) {
    ...
    if (allInGoal && !this.winnerRankings.includes(color)) {
      ...
      if (this.winnerRankings.length >= 3 || this.activeColors.length - this.winnerRankings.length <= 1) {
        // Automatically assign remaining unfinished player to last rank
        const remainingColor = this.activeColors.find(c => !this.winnerRankings.includes(c));
        if (remainingColor && !this.winnerRankings.includes(remainingColor)) {
          this.winnerRankings.push(remainingColor);
        }
    ...
    // 3. Reached GOAL
    let extraTurn = (this.lastDiceValue === 6 || !!captured || reachedGoal);
