...
// Cryptographically Secure Hardware Entropy RNG (Same as Ludo King / MPL / Casino RNG standards)
...
class LudoEngine {
  ...
  isUserInRoom(playerId, name, socketId) {
    ...
    for (const spec of this.spectators.values()) {
      if (spec && normName && spec.name && spec.name.trim().toLowerCase() === normName) return true;
    }
